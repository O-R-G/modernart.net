		...
		if ($imageName = $_FILES["upload".$i]["name"]) {
			...
			if ($copy) {
				...
				if ($resize) {
					...
					$mob_width = 568;
					$image_mob = new SimpleImage();
					$image_mob->load($target);
					$image_mob->resizeToWidth($mob_width);
	
					$targetPath_mob = "../MEDIA/_LO/";
					$target_mob = $targetPath_mob . $targetFile;
					
					$image_mob->save($target_mob);
					/* END NEW CODE */
