...
if ($action != "update") {
	...
	if ($object) {
		...
		$objectID = $myrow["id"];
		$i = 1;
		$sql = "SELECT * FROM media WHERE object = '". $objectID ."' AND active = '1' ORDER BY modified, created, id";
		$result = MYSQL_QUERY($sql);
		...
		while ($myrow = MYSQL_FETCH_ARRAY($result)) {
			...
			echo "\n<td><a href='$mediaFile' target='_blank'><img src='". $mediaFile ."' width='160' border='0'></a>";
	...
	for ($i = 0; $i < $uploadsMax; $i++) {
		...
		if ($imageName = $_FILES["upload".$i]["name"]) {
			...
			//echo "wants to upload |". $_FILES["upload". $i]['name'] ."| to |". $target ."| via |". $_FILES["upload". $i]['tmp_name'] ."|<br /><br />";
