// Modified by O-R-G on 01/03/2010
// Added arrow navigation 
// Swipe gesture in process
...
Scroller = new Class({
	...
	next: function(stop){
		...
		if (this.cChildIdx >= this.childs.length) {
			// this.cChildIdx = 0;
			this.cChildIdx = this.childs.length-1;		// Changed to cancel loop around
		}
		this.gotoIdx();
	},
	restart: function() {
	...
	prev: function(stop){
		...
		if (this.cChildIdx < 0) {
			...
			this.cChildIdx = 0;							// Changed to cancel loop around
		}
		this.gotoIdx();
	},
	gotoIdx: function () {
		...
		window.scrollTo(0,0); // Added this to auto-scroll to top of slide content
	}
});


		window.addEvent('keydown', function(event){
			if (event.key == 'right') this.Demo.scc.next();
			if (event.key == 'left') this.Demo.scc.prev();
			// if (event.key == 'down') this.Demo.scc.next();
			// if (event.key == 'up') this.Demo.scc.prev();
		});

		// Swipe gesture in process 
		
		// window.addEvent('swipe', 'this.Demo.scc.next()');
		// Element.Events.swipe.swipeWidth = 20;
