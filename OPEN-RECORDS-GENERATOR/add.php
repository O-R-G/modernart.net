...
if ($action != "add") {
	...
	for ($i = 0; $i < $uploadsMax; $i++) {
		...
		if ($imageName = $_FILES["upload".$i]["name"]) {
			...
			$resize = TRUE;
			$max_d = 2000; // maximum desktop dimension
			$max_m = 1000; // maximum mobile dimension
			...
			if ($copy) 
			{
				if ($resize) 
				{
					...
					try {
						$image = new abeautifulsite\SimpleImage($target);
					}
					catch(Exception $e) {
						echo "Caught exception: ", $e->getMessage(), "\n";
					}
					
					// medium-res, for desktop
					// $desktop_im = $image->best_fit($max_d, $max_d);
					$targetPath = "../MEDIA/"; //$dbMedia;
					$target = $targetPath . $targetFile;	
					$image->save($target, 70);
					
					// lo-res version, for mobile
					$mobile_im = $image->best_fit($max_m, $max_m);
					$targetPath = "../MEDIA/_LO/"; //$dbMedia;
					$target = $targetPath . $targetFile;	
					$mobile_im->save($target);
